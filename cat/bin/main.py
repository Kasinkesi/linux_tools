...
def parse_args():
    ...
    parser.add_argument("files", nargs="*", default=sys.stdin,
                        help="""перечисление файлов для чтения(объеинения). Если ФАЙЛ не указан или вместо его имени 
                        стоит дефис (-), то производится чтение со стандартного ввода.""")
    ...
    parser.add_argument("-s", "--smile-exterminator", dest="smile_exterminator", action="store_true",
                        help="заменяет все '8)' из выходного файла  на 'no smile for the wicked' "
                             "и делает доку грустным 8(")
